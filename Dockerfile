...
# ── Logos & favicons: /app/assets/img/ (source, used for inline SVG include) ─
...
# ── Logos & favicons: /app/build/img/ (served files, unhashed + hashed names) ─
# Unhashed
...
# Hashed filenames (fingerprinted by the asset pipeline — must overwrite these too)
